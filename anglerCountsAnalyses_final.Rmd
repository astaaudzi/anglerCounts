...
limts <- c(xmin =  .4 + min(g1$data$lon),
           xmax = -.4 + max(g1$data$lon),
           ymin =  .2 + min(g1$data$lat),
           ymax = -.2 + max(g1$data$lat)
)
...
fig1 <- ggdraw() +
    draw_plot(g1s) +
    draw_plot(g2, x = 0.15, y = 0.10, width = 0.4, height = 0.4)
...
# Deeper data in Kaunas
kaunas_sf <- readRDS( here("data","lithuania", "kaunas_sf.rds")) 


# Deeper rounded geometries, in drone dates

deeper_df <- kaunas_sf %>%
    dplyr::mutate(lon = sf::st_coordinates(.)[,1],
                  lat = sf::st_coordinates(.)[,2]) %>%
    dplyr::semi_join(drones_df, by= "date_name") %>% 
    dplyr::select(user_id, lat, lon, date_name, hour_of_day, duration) %>% 
    dplyr::mutate(lat = as.numeric(as.character(lat,4))) %>% 
    dplyr::mutate(lon = as.numeric(as.character(lon,4))) %>% 
    # group_by(trip_id) %>% 
    # filter(row_number()==1 | row_number()==n()) %>%  # filter first and last positions
    # ungroup() %>% 
    dplyr::mutate(type = "deeper")  %>% 
    # dplyr::mutate(across(c(user_id, date_name, trip_id,  type),factor)) %>% 
    # dplyr::filter(hour_of_day >= 6, hour_of_day <= 12) %>% 
    st_drop_geometry() 

# Deeper rounded geometries, in drone dates, between 6 - 12h

deeper612_df <- kaunas_sf %>%
    dplyr::mutate(lon = sf::st_coordinates(.)[,1],
                  lat = sf::st_coordinates(.)[,2]) %>%
    dplyr::semi_join(drones_df, by= "date_name") %>% 
    dplyr::select(user_id, lat, lon, date_name, hour_of_day, duration) %>% 
...
deeper_drone_df <- dplyr::bind_rows(drones_df, deeper_df) %>% 
    dplyr::distinct() %>% 
    drop_na(lat|lon) %>% 
    dplyr::mutate(across(c(user_id,  type),factor)) %>% 
    dplyr::mutate(date_name = ymd(date_name)) 


saveRDS(deeper_drone_df, here("data","lithuania", "deeper_drone_kaunas",  "deeper_drone_df.rds"))

# Hour filtered version

deeper_drone612_df <- dplyr::bind_rows(drones_df, deeper612_df) %>% 
    dplyr::distinct() %>% 
    drop_na(lat|lon) %>% 
    dplyr::mutate(across(c(user_id,  type),factor)) %>% 
    dplyr::mutate(date_name = ymd(date_name)) 
...
deeper_drone612_sf <- st_as_sf(deeper_drone612_df, coords = c("lon", "lat"),  crs=4326)

# saveRDS(deeper_drone_sf, here("data","lithuania", "deeper_drone_kaunas",  "deeper_drone_sf.rds"))
# 
# saveRDS(deeper_drone612_sf, here("data", "lithuania" , "deeper_drone_kaunas",  "deeper_drone612_sf.rds"))
...
deeper_drone_sf <- readRDS(here("data","lithuania", "deeper_drone_kaunas",  "deeper_drone_sf.rds"))
deeper_drone_surveyed_area_sf <- readRDS( here("data", "lithuania", "deeper_drone_kaunas",  "deeper_drone_surveyed_area_sf.rds"))
deeper_drone_surveyed_area_612_sf <- readRDS( here("data","lithuania", "deeper_drone_kaunas",  "deeper_drone_surveyed_area_612_sf.rds"))
...
unique(deeper_drone_surveyed_area_sf$date_name)
#  [1] "2020-03-23" "2020-03-28" "2020-04-24" "2020-04-25" "2020-05-05" "2020-05-09" "2020-05-15"
#  [8] "2020-05-21" "2020-05-26" "2020-05-30" "2020-09-02" "2020-09-11" "2020-09-19" "2020-09-24"
# [15] "2020-10-03" "2020-10-14" "2020-10-24" "2020-11-05" "2020-11-21" "2020-11-22" "2020-11-23"
# [22] "2020-11-24" "2020-11-25" "2020-11-26" "2020-11-27" "2020-11-28" "2020-11-29" "2020-11-30"
# [29] "2020-06-07" "2020-06-16" "2020-06-27" "2020-07-09" "2020-07-18" "2020-07-24" "2020-08-04"
# [36] "2020-08-14" "2020-08-22" "2020-08-28" "2020-12-09" "2020-12-13" "2020-12-27" "2021-01-21"
# [43] "2021-01-23" "2021-01-24" "2021-01-30" "2021-02-19" "2021-02-24" "2021-02-27"


map1 <- deeper_drone_sf %>%
    dplyr::filter(date_name >= "2021-01-24" & date_name <= "2021-01-24") %>% 
    leaflet() %>% 
    addTiles() %>%
    addCircles(weight = 2, 
    ...
    addLegend("bottomright", pal = pal, values = ~type,
              title = "Echosounder vs. drone observations, Kaunas reservoir",
              opacity = 1)

mapview::mapshot(map1, file = here("output", "deeper_drone_sf.png") )

### Drone surveyed area only

map2 <- deeper_drone_surveyed_area_sf %>%
    dplyr::filter(date_name >= "2021-01-24" & date_name <= "2021-01-24") %>% 
    leaflet() %>% 
    addTiles() %>%
    addCircles(weight = 2, 
    ...
    addScaleBar(position = "topright")  %>%
    addLegend("bottomright", pal = pal, values = ~type,
              title = "Echosounder vs. drone observations, drone surveyed area",
              opacity = 1)

mapview::mapshot(map2, file = here("output", "deeper_drone_surveyed_area_sf.png") )

#### Drone surveyed area, between 6-12h

map3 <- deeper_drone_surveyed_area_612_sf %>% 
    dplyr::filter(date_name >= "2021-01-24" & date_name <= "2021-01-24") %>% 
    leaflet() %>% 
    addTiles() %>%
    addCircles(weight = 2, 
    ...
    addScaleBar(position = "topright")  %>%
    addLegend("bottomright", pal = pal, values = ~type,
              title = "Echosounder vs. drone observations, drone surveyed area,between 6-12h",
              opacity = 1)

mapview::mapshot(map3, file = here("output", "deeper_drone_surveyed_area_612.png") )
...
multi3_df <- readRDS( here("data", "lithuania", "total_lithuania_df.rds")) %>% 
...
write.csv(kaunas3_df, here("data", "lithuania", "deeper_users", "kaunas3_df.csv"))
...
# To avoid the error below, swith off the spherical geometry:

#  Error in s2_geography_from_wkb(x, oriented = oriented, check = check) : 
#   Evaluation error: Found 1 feature with invalid spherical geometry.
# [1] Loop 0 is not valid: Edge 1 is degenerate (duplicate vertex).

sf::sf_use_s2(use_s2 = FALSE) #> Spherical geometry (s2) switched off

kaunas_drone_sf <- 
    sf::st_join(multi3_sf, drone_shp, join = st_within) %>% 
    # spatial join to get intersection of points and poly
    dplyr::filter(!is.na(NAM)) 
...
write.csv(kaunas3_drone_df,  here("data", "lithuania", "deeper_users", "kaunas3_drone_df.csv"))
...
write.csv(kaunas3612_drone_df, here("data", "lithuania", "deeper_users", "kaunas3612_drone_df.csv"))
...
## to fully propagate error we get lower and upper intervals also from using lower and upper intervals of angler proportion (in Table 2)
